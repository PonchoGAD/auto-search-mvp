...
MIN_TEXT_LEN = 40          # короткие структурированные посты (год, пробег, цена на отдельных строках)
MIN_BRAND_ONLY_LEN = 80
...
# Стоп-слова: только явные признаки НЕ-продажи (ремонт, запчасти, вопросы)
# НЕ блокируем слова, которые встречаются в описании продаваемого авто
# (двигатель, коробка, диски — люди их пишут при продаже)
STOP_WORDS = [
    "подписывайтесь", "вакансия", "работа", "акция", "обсуждение",
    "опрос", "новости", "ремонт", "диагностика", "ошибка",
    "запчасти", "разбор", "по запчастям",
    "подскажите", "вопрос", "приобрету",
    "сервис", "колодки", "ваносы", "ксентри", "xentry",
...
RE_PRICE = re.compile(
    r"(\d[\d\s\.]{1,15})\s*(млн|миллион|🍋|лимон|₽|руб|р\.?|тыс|k|к|\$|€|usd|eur)",
    re.IGNORECASE,
)
RE_PHONE = re.compile(r"\+?\d[\d\-\(\)\s]{8,}")
RE_MILEAGE = re.compile(r"\d[\d\s]{1,8}\s*км", re.IGNORECASE)
RE_YEAR = re.compile(r"\b(19[8-9]\d|20[0-2]\d)\b|(?<!\d)([89]\d|[012]\d)\s*(год|г\.?\s|\s)", re.IGNORECASE)  # 2018, 18г, 2020 год
